...
- A verified primary agent harness: Claude Code, Grok, Pi, `pi-signed`, Codex, OpenCode, or auggie.
...
Codex, OpenCode, and auggie are also verified and supported as primary harnesses; Codex uses bounded foreground checkpoints, OpenCode uses a TUI plugin, and auggie uses background-notify wake cycles with a passive turn-end resume backstop, so all three carry more harness-specific supervision tradeoffs than the three co-primaries.
...
- [docs/supervision-protocols/](docs/supervision-protocols/) - rendered primary-harness watcher protocols for Claude, Codex, OpenCode, Pi and `pi-signed`, Grok, auggie, and unknown harness fallback.
