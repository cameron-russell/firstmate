...
case "$HARNESS" in
  claude|codex|opencode|pi|grok|auggie) SNIPPET="$DOC_DIR/$HARNESS.md" ;;
...
repair_line() {
  ...
  case "$HARNESS" in
    ...
    grok)
      ...
      ;;
    auggie)
      printf '%s%s\n' "$prefix" 'repair missing watcher supervision with bin/fm-watch-arm-auggie.sh as its own launch-process background task (wait: false), never shell &.'
      ;;
    *)
...
ordinary_wake_line() {
  case "$HARNESS" in
    ...
    grok)
      ...
      ;;
    auggie)
      printf '%s\n' '- Ordinary wake: re-arm exactly one bin/fm-watch-arm-auggie.sh launch-process background task (wait: false) as directed below.'
      ;;
    *)
