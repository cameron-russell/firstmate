...
secondmate_liveness_sweep() {
  ...
  for meta in "$STATE"/*.meta; do
    ...
    case "$harness" in
      claude|codex|opencode|pi|pi-signed|grok|kimi|auggie) ;;
      *)
...
crew_dispatch_validate() {
  ...
  err=$(jq -r '
    def verified($h): ["claude","codex","opencode","pi","pi-signed","grok","kimi","muse","auggie"] | index($h);
    def effort_ok($h; $e):
      if $e == null then true
      ...
      elif $h == "opencode" or $h == "kimi" or $h == "auggie" then false
      else true
      end;
    def profiles($value):
