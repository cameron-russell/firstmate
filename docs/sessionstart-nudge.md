...
| auggie | Nudge | The tracked repo-root `.augment/settings.json` registers a `SessionStart` hook and invokes the wrapper through inline-defaulted `${AUGMENT_PROJECT_DIR:-}`. | The hook runs with no separate folder-trust step, but auggie 0.34.0 does not inject `SessionStart` hook stdout into interactive TUI model context, so this path is intentionally fail-open and cannot use the run tier; the emitted operating block plus the read-once reminder carry the nudge. |
