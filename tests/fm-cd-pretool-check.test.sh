...
run_matrix_entry() {
  ...
  case "$entry" in
    ...
    grok)
      ...
      ;;
    auggie)
      payload=$(jq -cn --arg command "$cmd" '{tool_name:"launch-process",tool_input:{command:$command}}')
      printf '%s' "$payload" | "$CHECK" --auggie >"$out_file" 2>"$err_file"
      rc=$?
      ;;
    opencode|pi)
  ...
  elif [ "$entry" = auggie ]; then
    jq -e '.hookSpecificOutput.permissionDecision == "deny" and (.hookSpecificOutput.permissionDecisionReason | test("\\[persistent-cd\\]"))' "$out_file" >/dev/null 2>&1 \
      || fail "$id via auggie deny must carry hookSpecificOutput.permissionDecision=deny with the reason on stdout: $(cat "$out_file")"
    jq -e 'has("decision")' "$out_file" >/dev/null 2>&1 \
      && fail "$id via auggie deny must NOT also emit the Grok decision object: $(cat "$out_file")"
  fi
}
...
test_full_acceptance_matrix() {
  ...
  for ((i = 0; i < ${#MATRIX_IDS[@]}; i++)); do
    for entry in codex claude grok auggie opencode pi; do
...
test_auggie_wiring() {
  local settings pre0 pre1 both
  settings="$ROOT/.augment/settings.json"
  [ -f "$settings" ] || fail "tracked repo-root .augment/settings.json is missing"
  # Both PreToolUse seatbelts must be present, both empty-matcher, both --auggie.
  jq -e '[.hooks.PreToolUse[] | select(has("matcher") | not)] | length >= 2' "$settings" >/dev/null \
    || fail "auggie cd hook must be an empty-matcher PreToolUse entry so it applies to every tool"
  pre0=$(jq -r '.hooks.PreToolUse[0].hooks[0].command // empty' "$settings")
  pre1=$(jq -r '.hooks.PreToolUse[1].hooks[0].command // empty' "$settings")
  both="$pre0$pre1"
  assert_contains "$both" 'fm-cd-pretool-check.sh" --auggie' "auggie must wire the cd-guard in --auggie mode"
  assert_contains "$both" 'fm-arm-pretool-check.sh" --auggie' "auggie cd hook must not displace the watcher-arm hook"
  assert_contains "$both" 'AUGMENT_PROJECT_DIR' "auggie cd hook must anchor from AUGMENT_PROJECT_DIR"
  pass ".augment/settings.json: PreToolUse invokes the cd-guard alongside the arm guard in --auggie mode"
}
